        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Tambah Data Transaksi Detail</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{ route('admin.transaksi.store.detail', $transaksi_siswa->id) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                            <div class="form-group">
                                <label for="exampleInputEmail1">Nama Pembayaran</label>
                                <select name="pembayaran" class="form-control" @error('pembayaran') is-invalid @enderror required>
                                    <option value="">-- Pilih Nama Pembayaran --</option>
                                    @foreach ($pembayaran as $pmbyrn)
                                    <option value="{{ $pmbyrn->id }}">{{ $pmbyrn->nama }}</option>
                                    @endforeach
                                </select>
                                <div class="invalid-feedback">
                                    Masukan Nama Pembayaran
                                </div>
                            </div>
                        </div>
